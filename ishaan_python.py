...
def parse_pdf(pdf_file):
    ...
    with open("temp_uploaded_criteria.pdf", "wb") as f:
        f.write(pdf_file.read())

    loader = PyMuPDFLoader("temp_uploaded_criteria.pdf")
    docs = loader.load()
...
with tab1:
    ...
    if not st.session_state.volunteers_df.empty:
        ...
        st.info("Please upload a Volunteers CSV to proceed.")
...
with tab2:
    ...
    if st.session_state.criteria and not st.session_state.volunteers_df.empty:
        ...
        gender2 = st.selectbox("Gender", ["All"] + sorted(st.session_state.volunteers_df["Gender"].unique()), 
                               key="med_gender")
        region2 = st.selectbox("Region", ["All"] + sorted(st.session_state.volunteers_df["Region"].dropna().unique()), 
                               key="med_region")
    ...
    elif st.session_state.volunteers_df.empty:
        st.info("Please upload a Volunteers CSV in the TechVitals Admin tab first.")
