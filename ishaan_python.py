...
def gpt_answer(q, df):
    ...
    prompt = PromptTemplate(
        template="""You are an AI assistant helping to answer queries about clinical trial volunteers.
...
# -------------------------- Q&A Function for TechVitals --------------------------
def answer_question(q, df, criteria=None):
    q_lower = q.lower()

    if "summary" in q_lower or "data" in q_lower:  # Handle summary query for TechVitals
        return f"TechVitals has provided a dataset with the following columns: {', '.join(df.columns)}.\nTotal volunteers: {len(df)}.\nThis dataset includes all volunteers without any filters applied."
        ...
        return gpt_answer(q, df)  # Use GPT for fallback if rule-based fails

# -------------------------- Q&A Function for Medical Companies --------------------------
def answer_medical_company_question(q, df, criteria=None):
    q_lower = q.lower()

    if "summary" in q_lower or "data" in q_lower:  # Handle summary query for Medical Companies
        return f"TechVitals provided a dataset with columns: {', '.join(df.columns)}.\nMedical companies provided the criteria: {criteria}\nTotal number of eligible volunteers: {len(df)}."
...
with tab2:
    ...
    if st.session_state.criteria:
        ...
        if st.button("Ask", key="medical_ask"):
            ...
            ans2 = answer_medical_company_question(q2, filtered_med, st.session_state.criteria)
