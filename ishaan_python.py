...
def answer_question(q, df, criteria=None):
    ...
    if "how many" in q_lower:
        ...
        if "above age" in q_lower:
            age = int(re.findall(r'age (\d+)', q_lower)[0])
...
with tab2:
    ...
    if st.session_state.criteria:
        ...
        st.dataframe(eligible[["VolunteerID", "Email", "DiseaseStage", "BiomarkerStatus", "Gender"]])

        st.subheader("Ask a Question (AI Powered)")
        q2 = st.text_input("Ask about eligible volunteers:", key="medical_q")
        if st.button("Ask", key="medical_ask"):
            ans2 = answer_question(q2, eligible, st.session_state.criteria)
