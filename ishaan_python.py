...
def answer_question(q, df, criteria=None):
    ...
    if "how many" in q_lower:
    ...
    elif "why is volunteer" in q_lower and "eligible" in q_lower:
        vid = re.findall(r'volunteer (\w+)', q_lower)
        if vid:
            vid = vid[0]
            row = df[df["VolunteerID"] == vid]

            if row.empty:
                return f"Volunteer ID {vid} not found in the dataset."

            if vid in df["VolunteerID"].values:
                return f"Volunteer {vid} is eligible for clinical trials."
            else:
                reasons = []
                r = row.iloc[0]
        ...
        return None  # GPT can provide an answer if rule-based fails
...
with tab1:
    ...
    if df.empty:
        ...
        # Displaying summary for TechVitals Admin
        st.write("TechVitals have provided a dataset with columns such as 'VolunteerID', 'Age', 'Gender', 'Condition', 'BiomarkerStatus', and more.")
        st.write(f"Total volunteers are: {len(df)}")
...
with tab2:
    ...
    if "volunteers_df" not in st.session_state:
        st.write("Waiting for TechVitals to add the dataset...")
    ...
    if st.session_state.criteria:
        ...
        min_age_m, max_age_m = st.slider("Age Range", 0, 100, (st.session_state.criteria["min_age"], st.session_state.criteria["max_age"]), key="medical_age")
        ...
        st.dataframe(filtered_med[["VolunteerID", "Email", "DiseaseStage", "BiomarkerStatus", "Gender", "Region", "Age"]])
