...
with tab2:
    ...
    if not st.session_state.volunteers_df.empty:
        st.subheader("TechVitals Summary")
        st.markdown(f"TechVitals has provided a dataset with columns like {', '.join(df.columns)}. "
                    f"The total number of volunteers in the dataset is {len(df)}.")
